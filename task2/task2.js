...
const func = (resources) => {

    let campaignEU = resources.filter((el) => el.type == "campaign" && el.region == "eu").map((el) => (
        {
            id: el.id,
            name: el.name,
            campaign_id: el.code.split("_")[1],
            country: el.code.split("_")[0].toUpperCase(),
            startDate: el.startDate,
            endDate: el.endDate
        }
    )
    )
    let campaignLATAM = resources.filter((el) => el.type == "campaign" && el.region == "latam").map((el) => (
        {
            id: el.id,
            name: el.name,
            campaign_id: el.code.split("_")[1],
    ...
    let orderEU = resources.filter((el) => el.type == "order" && el.region == "eu").map((el) => (

        {
            id: el.id,
            name: el.name,
            order_id: el.code.split("_")[1],
            country: el.code.split("_")[0].toUpperCase(),
            startDate: el.startDate,
            endDate: el.endDate
        }
    ...
    let orderNA = resources.filter((el) => el.type == "order" && el.region == "na").map((el) => (
    ...
    let orderAPAC = resources.filter((el) => el.type == "order" && el.region == "apac").map((el) => (
    ...
    let orderLATAM = resources.filter((el) => el.type == "order" && el.region == "latam").map((el) => (
